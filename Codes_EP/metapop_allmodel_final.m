% Meta-pop model for 66 colonies
% - You can find codes for the metapopulation model for CMR and IPM model and for all climate models. 
% - You can also find codes for projections between 2009 and 2100.
% - There are codes to apply the metapopulation model to the mean of ensembles (intrisic climate variability)

clear all; clc; close all;

rand('state', sum(100.*clock)) % reset of the rand function

% DIRECTORY to Codes_EP folder
ordi = '/Users/aliceeparvier/Desktop';


% Metapopulation model for CMR and IPM

%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
%% Initialization
%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

load(sprintf('%s/Codes_EP/Codes_Stef/mat_data/infos_models.mat', ordi)) % file with info on climate models (dates, ensemble members)

% number of simulation
nsimulation=1000; %8000 %should be smaller than 8000 because 8000 posterior for dispersive data

dataOBSSAT_bilgecan=readmatrix(sprintf('%s/Codes_EP/Codes_Stef/DataOBS_SAT_BILGECAN/N_glb.csv', ordi)); % Observed satellite data
...
% Adjust initial conditions to observations
BE=0.85*BE; % for CMR 1900
...
%%%% calculate costal distance between colonies in km
...
%% SIMULATIONS (for CMR, IPM, with all climate models)

% CHOOSE ecological model
mod_pop= ["Stef", "IPM"]; % ecological model, CMR or IPM
e_mod=2;
...
if e_mod==1 %model CMR
    scen_list = [1 2 4]; %extreme event scenarios
    mod_list=[2:3 7]; % climate models
else % IPM doesn't have scenEXT
    scen_list = [1];
    mod_list=6;
end
...
for m=mod_list %climate model
    ...
    nt        = length(timePOP);
    ncol=66;
    nens= info_models(m,5); %ensemble members
    nsim=100;
    ...
    for scenEXT=scen_list
        ...
        for ens=1:nens
            ...
            % Import data of all the colonies (Growth rates)
            parfor col=1:ncol
                if e_mod==1
            ...
            for col=1:ncol
                ...
                % MAT files give the proportion of mature individuals in
            ...
            % to calculate growth rate after dispersion
...
% Natural climate variability. It uses GR obtained with the mean of
% ensemble members.
...
% INITIAL conditions
BE=1.7*BE; %so that mature fits obs CMR
BE=1.2*BE %IPM
